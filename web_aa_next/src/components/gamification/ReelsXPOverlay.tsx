...
export const ReelsXPOverlay: React.FC<ReelsXPOverlayProps> = ({ state }) => {
  ...
  // XP artışı varsa progress bar'ı göster (3 saniye)
  useEffect(() => {
  ...
  const nodeProgress = (state.currentXP / 100) * 100; // 0-100%
    ...
    <div className="px-4 py-2.5 bg-gradient-to-b from-black/70 via-black/30 to-transparent pointer-events-none">
      ...
      <div className="inline-flex items-center space-x-3 px-3 py-2 bg-black/60 rounded-full border border-white/20 pointer-events-none">
        ...
        <div className="flex items-center space-x-0.5">
          {Array.from({ length: state.nodesInLevel }).map((_, index) => {
            const isCompleted = index < state.currentNode;
            const isCurrent = index === state.currentNode;
            ...
            return (
              <div
                ...
                className={clsx(
                  ...
                  isCurrent ? 'w-2.5 h-2.5 border-2 border-amber-300' : 'w-2 h-2',
                  isCompleted ? 'bg-amber-400' : isCurrent ? 'bg-amber-200' : 'bg-white/30'
                )}
              />
            );
          })}
      ...
      >
        <div className="flex flex-col">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="text-amber-300 text-[11px] font-bold">{Math.round(nodeProgress)}%</span>
          </div>
